...
var myData = JSON.parse(pagination);
var rowPerPage = 8;
var noOfPages = Math.ceil(myData.length / rowPerPage);
var currentPage = 1;
var prevPage = 1;
var nextPage = 2;
...
function createTable(columns) {
  ...
  for (let i = 0; i < columns.length; i++) {
    ...
    tableHeadRowData.innerHTML = columns[i];
...
function createPaginationRow(noOfPages, rowPerPage) {
  const paginateRow = document.createElement("ul");
  paginateRow.setAttribute("class", "pagination");

  paginateRow.appendChild(createPaginationElement("Previous"));
  paginateRow.appendChild(createPaginationElement("Next"));
  for (let j = 1; j <= noOfPages; j++) {
    paginateRow.appendChild(createPaginationElement(j));
  }

  document.querySelector("body").appendChild(paginateRow);
}

function createRowElement(id, name, email) {
  ...
  rowDataName.innerHTML = name;

  const rowDataEmail = document.createElement("td");
  rowDataEmail.innerHTML = email;

  row.append(rowHead, rowDataName, rowDataEmail);

  return row;
}

function createPaginationElement(pageNumber) {
  ...
  pageLink.setAttribute("onclick", `paginate("${pageNumber}")`);
...
function paginate(pageNumber) {
  if (pageNumber === "Next")
    currentPage = currentPage != noOfPages ? currentPage + 1 : currentPage;
  else if (pageNumber === "Previous")
    currentPage = currentPage != 1 ? currentPage - 1 : currentPage;
  else currentPage = parseInt(pageNumber);

  const rangeFrom = currentPage === 1 ? 0 : (currentPage - 1) * rowPerPage;
  ...
  for (
    let i = rangeFrom;
    i < rangeFrom + rowPerPage && i < myData.length;
    i++
  ) {
    tbody.appendChild(
      createRowElement(myData[i].id, myData[i].name, myData[i].email)
    );
  }
...
function load() {
  ...
  body.append(createTable(columns));
  createPaginationRow(noOfPages);
